...
test('Phase 6: job stepper renders visual stage cards with active progress accent', () => {
  ...
  assert(indexHtml.includes('.job-stepper .step-spinner'), 'Active step should have a spinner style');
  assert(indexHtml.includes('@keyframes step-spin'), 'Active step spinner should animate clockwise');
  ...
  assert(script.includes("const spinner = i === activeIdx ? '<span class=\"step-spinner\""),
    'Active step (any phase) should render the spinner next to its name');
